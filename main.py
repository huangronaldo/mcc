...
class JDWrapper(object):
    ...
    def response_status(resp):
        if resp.status_code != requests.codes.OK:
            print
            'Status: %u, Url: %s' % (resp.status_code, resp.url)
            return False
        return True
    ...
    def _login_try(self):
        ...
        try:
            ...
            if login_succeed:
                ...
                print
                u'登陆成功 %s' % self.usr_name
            else:
                print
                u'登陆失败 %s' % self.usr_name
    ...
    def login_by_QR(self):
        ...
        try:
            ...
            '+++++++++++++++++++++++++++++++++++++++++++++++++++++++'
            print
            u'{0} > 请打开京东手机客户端，准备扫码登陆:'.format(time.ctime())
            ...
            while retry_times:
                ...
                if rs['code'] == 200:
                    print
                    u'{} : {}'.format(rs['code'], rs['ticket'])
                    qr_ticket = rs['ticket']
                    break
                else:
                    print
                    u'{} : {}'.format(rs['code'], rs['msg'])
                    time.sleep(3)
            ...
            if resp.status_code != requests.codes.OK:
                print
                u'二维码登陆校验失败: %u' % resp.status_code
                return False
    ...
    def good_detail(self, stock_id, area_id=None):
        ...
        '+++++++++++++++++++++++++++++++++++++++++++++++++++++++'
        print
        u'{0} > 商品详情'.format(time.ctime())
        print
        u'编号：{0}'.format(good_data['id'])
        print
        u'库存：{0}'.format(good_data['stockName'])
        print
        u'价格：{0}'.format(good_data['price'])
        print
        u'名称：{0}'.format(good_data['name'])
    ...
    def buy(self, options):
        ...
        if good_data['stock'] != 33:
            ...
            while good_data['stock'] != 33 and options.flush:
                print
                u'<%s> <%s>' % (good_data['stockName'], good_data['name'])
        ...
        try:
            ...
            '+++++++++++++++++++++++++++++++++++++++++++++++++++++++'
            print
            u'{0} > 购买详情'.format(time.ctime())
            print
            u'链接：{0}'.format(link)
            print
            u'结果：{0}'.format(tags_val(tag))
    ...
    def cart_detail(self):
        ...
        try:
            ...
            for item in soup.select('div.item-form'):
                ...
                #: ￥字符解析出错, 输出忽略￥
                print
                cart_format.format(check, count, price[1:], sums[1:], gname)

            t_count = tags_val(soup.select('div.amount-sum em'))
            t_value = tags_val(soup.select('span.sumPrice em'))
            print
            u'总数: {0}'.format(t_count)
            print
            u'总额: {0}'.format(t_value[1:])
    ...
    def order_info(self, submit=False):
        ...
        try:
            ...
            if rp.status_code == 200:
                ...
                if js['success'] == True:
                    print
                    u'下单成功！订单号：{0}'.format(js['orderId'])
                    print
                    u'请前往东京官方商城付款'
                    return True
                else:
                    print
                    u'下单失败！<{0}: {1}>'.format(js['resultCode'], js['message'])
                    if js['resultCode'] == '60017':
                        # 60017: 您多次提交过快，请稍后再试
                        time.sleep(1)
            else:
                print
                u'请求失败. StatusCode:', rp.status_code
